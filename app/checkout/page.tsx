...
function CheckoutContent() {
  ...
  // Tuesday at the shop. After the 2PM Cambodia cutoff the florist can no
  // longer deliver the same day, so the earliest offered is tomorrow.
  const [shopTodayDate, setShopTodayDate] = useState('')
  const [earliestDate, setEarliestDate] = useState('')
  const [pastCutoff, setPastCutoff] = useState(false)
  const [shopClock, setShopClock] = useState('')
  useEffect(() => {
    const sync = () => {
      setShopTodayDate(shopToday())
      setEarliestDate(earliestDeliveryDate())
      setPastCutoff(isPastSameDayCutoff())
      setShopClock(shopTimeLabel())
    }
    sync()
    // Keep it honest if the page sits open across the cutoff.
    const t = setInterval(sync, 60000)
    return () => clearInterval(t)
  }, [])

  // Drop a date that the cutoff has just put out of reach.
  useEffect(() => {
    if (earliestDate && deliveryDate && deliveryDate < earliestDate) setDeliveryDate('')
            ...
            <div style={{ background: '#fff', borderRadius: 16, border: '1px solid #FFE4EF', padding: 24 }}>
              ...
              <div className="grid-form-2col">
                ...
                <div>
                  ...
                  <input type="date" value={deliveryDate} onChange={e => setDeliveryDate(e.target.value)} style={inputStyle} min={earliestDate || undefined} />
                </div>
                <div>
                ...
                <div style={{ gridColumn: '1 / -1', marginTop: -4 }}>
                  <div style={{ fontSize: 12, color: '#9C7A8E', background: '#FFF8FC', border: '1px solid #FFE4EF', borderRadius: 10, padding: '10px 14px', lineHeight: 1.6 }}>
                    ...
                    {shopTodayDate && <> It&apos;s <strong>{formatShopDate(shopTodayDate)}, {shopClock}</strong> in Phnom Penh right now.</>}
                    {earliestDate && (
                      pastCutoff
                        ? <> Same-day orders close at {SAME_DAY_CUTOFF_LABEL} Cambodia time, so the earliest delivery is <strong>{formatShopDate(earliestDate)}</strong>.</>
                        : <> Order before {SAME_DAY_CUTOFF_LABEL} Cambodia time for same-day delivery.</>
                    )}
                  </div>
                </div>
                <div style={{ gridColumn: '1 / -1' }}>
